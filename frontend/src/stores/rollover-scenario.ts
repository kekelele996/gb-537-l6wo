...
interface RolloverScenarioState {
  ...
  signoffRisk: (id: number, input: RiskSignoffInput) => Promise<RolloverScenario>
...
export const useRolloverScenarioStore = create<RolloverScenarioState>((set, get) => {
  ...
  return {
    ...
    signoffRisk: async (id, input) => { const updated = await rolloverScenarioApi.signoffRisk(id, input); merge(updated); return updated },
